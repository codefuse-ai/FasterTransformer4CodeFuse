...
void DynamicDecodeLayer<T>::forward(TensorMap* output_tensors, TensorMap* input_tensors)
{
     ...
     *   \param  optional_last_tokens [batch_size, max_optional_last_tokens_count], optional
    ...
    if (step == max_input_length && input_tensors->isExist("optional_last_tokens")) {

        const auto& optional_last_tokens = input_tensors->at("optional_last_tokens");

        const int max_optional_last_tokens_count = optional_last_tokens.shape[1];

        const int* optional_last_tokens_ptr =
            optional_last_tokens.getPtrWithOffset<const int>(ite * local_batch_size * max_optional_last_tokens_count);

        invokeSelectOptionalLastTokens(
            (T*)input_tensors->at("logits").getPtrWithOffset(ite * local_batch_size * beam_width * vocab_size_padded_),
            optional_last_tokens_ptr,
            local_batch_size,
            beam_width,
            vocab_size_padded_,
    ...
    if (input_tensors->isExist("bad_words_list")) {
        ...
        // TODO: bug? decode_vocab_size_units_offset didn't *= beam_width
